...
import {Async, AsyncHelper, PersonDataStatistics} from '../Model';
...
export function PersonSearchPage() {
    ...
    const [local, setLocal] = useState<QueryResults[]>();
    const [filterCounter, setFilterCounter] = useState<FilterCounter>({
        salary: 0,
        assets: 0,
        pytyvo: 0,
        nangareko: 0,
        ...
        new SimpleApi().findPeopleByName(cedula)
            .then(d => {
                ...
                Object.keys(d.data).forEach((ci: string) => {
                    ...
                    let netWorth: number | undefined = undefined;
                    let sources: string[] = [];
                    d.data[ci].forEach(row => {
                        if (row.source === 'declarations') netWorth = row.net_worth!;
                        sources.push(row.source);
                    });
                    results.push({document: document, name: name, sources: sources, netWorth, salary: 0});
                })
                setLocal(results);
                setData(results);
            });
    ...
    useEffect(() => {
        ...
        let newData: QueryResults[] = local;
        appliedFilters.forEach(filter => {
            if (filter.key === 'Salario') {
                newData = newData.filter(e => (e.salary || 0) < (filter.to || 0) && (e.salary || 0) > (filter.from || 0))
            } else if (filter.key === 'Patrimonio Neto') {
                newData = newData.filter(e => (e.netWorth || 0) < (filter.to || 0) && (e.netWorth || 0) > (filter.from || 0))
            } else {
    ...
    return <>
        ...
        <Layout>
            <Layout.Sider width={400} className="hide-responsive">
                <FilterMenu spans={{xxl: 24, xl: 24, lg: 24, md: 24, sm: 24, xs: 24}} cardsWidth={300}
                            filterCallback={applyFilterHandler}
                            counters={filterCounter}
                            sorter={sortResultHandler}/>
            </Layout.Sider>
            <Layout>
                <Layout.Content className="content-padding">
                    <Row gutter={[8, 16]}>
                        <Col className="show-responsive" xxl={{offset: 0, span: 20}} xl={{offset: 0, span: 20}}
                             lg={{offset: 0, span: 20}} md={{offset: 0, span: 20}} sm={{offset: 1, span: 22}}
                             xs={{offset: 1, span: 22}} style={{textAlign: 'left'}}>
                            <Collapse defaultActiveKey={['1']}>
                                <Collapse.Panel header="Filtros" key="1">
                                    <FilterMenu spans={{xxl: 24, xl: 24, lg: 24, md: 24, sm: 24, xs: 24}}
                                                cardsWidth={250} filterCallback={applyFilterHandler}
                        ...
                        <Col xxl={{offset: 0, span: 20}} xl={{offset: 0, span: 20}} lg={{offset: 0, span: 20}}
                            ...
                            <Card className="card-layout-content">
                                <Typography.Text className="text-layout-content">
                                    ...
                                    <Tag key={filter.key} closable
                                         onClose={() => deleteFilterHandler(filter)}>{filter.name} </Tag>
                                )}
                                </Typography.Text>
                            </Card>
                        </Col>
                        ...
                        <Col xxl={{offset: 0, span: 20}} xl={{offset: 0, span: 20}} lg={{offset: 0, span: 20}}
                             md={{offset: 0, span: 20}} sm={{offset: 1, span: 20}} xs={{offset: 1, span: 20}}
                             style={{textAlign: 'left'}}>
                        ...
                        </Col>
                        {
                            data && <>
                              <Col xxl={{offset: 0, span: 20}} xl={{offset: 0, span: 20}} lg={{offset: 0, span: 20}}
                                ...
                                {data.map(row =>
                                    <Col xxl={{offset: 0, span: 20}} xl={{offset: 0, span: 20}}
                                         lg={{offset: 0, span: 20}} md={{offset: 0, span: 20}}
                                         sm={{offset: 0, span: 24}} xs={{offset: 0, span: 24}}
                                         style={{textAlign: 'left'}}
                                         key={row.document}>
                                        <Card className="card-layout-content">
                                            <Row gutter={[8, 16]}>
                                                ...
                                                <Col xxl={7}
                                                     ...
                                                     xs={18}>
                                                    {<Typography.Text
                                                        className="text-layout-content"> {row.name} </Typography.Text>}
                                                </Col>
                                                <Col xxl={0} xl={0} lg={0} md={6} sm={6} xs={6}>
                                                    <Typography.Text className="text-layout-content"
                                                                     strong> Salario: </Typography.Text>
                                                </Col>
                                                <Col xxl={5}
                                                     xl={5}
                                                     lg={5}
                                                     md={18} sm={18} xs={18}>
                                                    <Typography.Text
                                                        className="text-layout-content"> </Typography.Text>
                                                </Col>
                                                <Col xxl={0} xl={0} lg={0} md={6} sm={6} xs={6}>
                                                    <Typography.Text className="text-layout-content"
                                                                     strong> Patrimonio Neto: </Typography.Text>
                                                </Col>
                                                <Col xxl={5}
                                                     xl={5}
                                                     lg={5}
                                                     md={18} sm={18} xs={18}>
                                                    <Typography.Text
                                                        className="text-layout-content"> {formatMoney(row.netWorth)} </Typography.Text>
                                                </Col>
                                                <Col xxl={5}
                                                     xl={5}
                                                     lg={5}
                                                     md={24} sm={24} xs={24}>
                                                    {row.sources.indexOf('sfp') !== -1 &&
                                                    <Tooltip title="SFP"><Icon component={Sfp} style={{
                                                        color: 'rgba(0, 52, 91, 1)',
                                                        fontSize: '30px'
                                                    }}/></Tooltip>}
                                                    {row.sources.indexOf('declarations') !== -1 &&
                                                    <Tooltip title="Declaraciones Juradas"> <Icon
                                                      component={Ddjj}
                                                      style={{color: 'rgba(0, 52, 91, 1)', fontSize: '30px'}}/>
                                                    </Tooltip>}
                                                    {row.sources.indexOf('pytyvo') !== -1 &&
                                                    <Tooltip title="Pytyvo"> <Icon component={Pytyvo} style={{
                                                        color: 'rgba(0, 52, 91, 1)',
                                                        fontSize: '30px'
                                                    }}/> </Tooltip>}
                                                    {row.sources.indexOf('nangareko') !== -1 &&
                                                    <Tooltip title="Ñangareko"> <Icon component={Nangareko}
                                                                                      style={{
                                                                                          color: 'rgba(0, 52, 91, 1)',
                                                                                          fontSize: '30px'
                                                                                      }}/> </Tooltip>}
                                                    {row.sources.indexOf('ande_exonerados') !== -1 &&
                                                    <Tooltip title="ANDE"> <Icon component={Ande} style={{
                                                        color: 'rgba(0, 52, 91, 1)',
                                                        fontSize: '30px'
                                                    }}/> </Tooltip>}
                                                    {row.sources.indexOf('policia') !== -1 &&
                                                    <Tooltip title="Policia Nacional"> <Icon
                                                      component={PoliciaNacional}
                                                      style={{color: 'rgba(0, 52, 91, 1)', fontSize: '30px'}}/>
                                                    </Tooltip>}
                                                </Col>
                                                <Col xxl={2}
...
function Statistic(props: {
    ...
    return <Tooltip placement="top" title={observation}>
        <Col className="info-box">
            <Typography.Text className="text-layout-content"><strong>{props.name}</strong> <span
                className={numberClasses}>{formatMoney(number)}</span></Typography.Text>
        </Col>
    </Tooltip>
}
...
function FilterMenu(props: {
    ...
    return <Row className="cards" gutter={[16, 24]}>
        <Col xxl={{offset: 1, span: 20}} xl={{offset: 1, span: 20}} lg={{offset: 1, span: 20}}
             md={{offset: 1, span: 20}} sm={{offset: 1, span: 20}} xs={{offset: 1, span: 20}}>
            ...
            <Card size="small"
                ...
                <Row gutter={[8, 16]}>
                    <Col {...props.spans}>
                        <Typography.Text> <Button type="text" style={{color: 'rgba(0, 52, 91, 1)'}}
                                                  onClick={() => sort('salary')}>Salario</Button> </Typography.Text>
                        <Badge className="float-right" count={props.counters.salary}
                               style={{backgroundColor: '#b2c1cc'}}/>

                    </Col>
                    <Col {...props.spans}>

                        <Typography.Text> <Button type="text" style={{color: 'rgba(0, 52, 91, 1)'}}
                                                  onClick={() => sort('networth')}>Patrimonio Neto</Button>
                        </Typography.Text>
                        <Badge className="float-right" count={props.counters.assets}
                               style={{backgroundColor: '#b2c1cc'}}/>
                    </Col>
                </Row>
            </Card>
                    ...
                    <Row gutter={[8, 16]}>
                        <Col {...props.spans}>
                            <Select style={{width: '100%'}} defaultValue={selected} onChange={setSelected}>
                                {filterOptions.map(k => <Select.Option value={k.key}
                                                                       key={k.key}>{k.name} </Select.Option>)}
                            </Select>
                        </Col>
                        <Col {...props.spans}>
                            <InputNumber type="number" style={{width: '100%'}} placeholder="Desde" defaultValue={from}
                                         onChange={onChange}/>
                        </Col>
                        <Col {...props.spans}>
                            <InputNumber type="number" style={{width: '100%'}} placeholder="Hasta" defaultValue={to}
                                         onChange={changeTo}/>
                        </Col>
                        <Col {...props.spans}>
                            <Button className="mas-button" style={{float: "right"}}
                                    onClick={applyRangeFilter}>Aplicar</Button>
                        </Col>
                    </Row>
                </Input.Group>
            </Card>
                    ...
                    </Col>
                    {
                        filtered.map(option =>
                            <Col {...props.spans} key={option.name}>
                                <Typography.Text><Button type="text" style={{color: 'rgba(0, 52, 91, 1)'}}
                                                         onClick={() => addFilter(option.key)}>{option.name}</Button></Typography.Text>
