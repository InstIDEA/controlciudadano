    ...
    return <Col {...props.colProps}>
        <Card className="data-box" title={<a href={AQE_URL}>aquieneselegimos.org.py</a>}
              ...
              actions={[
                  ...
                  <a href={`${AQE_URL}/detalle-persona/${person.id}`} target="_blank" rel="noopener noreferrer">Más
                      info</a>
              ]}>
            <Space direction="vertical">
                {person.date_of_death && person.date_of_death.length > 1 ?
                    <LVRow label={"Fecha de muerte"} value={person.date_of_death}/> : ''}
                {person.national_identity ?
                    <LVRow label={"País de nacimiento"} value={PAISES_NACIMIENTO[person.national_identity]}/> : ''}
                {person.city_of_residence ?
                    <LVRow label={"Ciudad de residencia"} value={person.city_of_residence}/> : ''}
                {person.estado_civil ?
                    <LVRow label={"Estado civil"} value={ESTADOS_CIVILES[person.estado_civil]}/> : ''}
